...
def row_gnn_after_start_fn(
	...
	if pd is None:  # pragma: no cover
		raise ModuleNotFoundError(
			"pandas is required for the FTTransformer start-stage row-GNN injection. Install pandas to use this feature."
	...
	# - 'split' (default): standardize each split independently (matches ExcelFormer port).
	# - 'train': standardize all splits using train statistics (may hurt under distribution shift).
...
def feature_gnn_after_start_fn(
	...
	if pd is None:  # pragma: no cover
		raise ModuleNotFoundError(
			"pandas is required for the FTTransformer feature-level placeholder. Install pandas to use this model."
		) from _PANDAS_IMPORT_ERROR
...
def materialize_fn(train_df, val_df, test_df, dataset_results, config):
	...
	try:
		...
		raise ModuleNotFoundError(
			"Failed to import local torch_frame components required by FTTransformer materialize_fn."
		) from e
	...
	try:
		...
		raise ModuleNotFoundError(
			"torchmetrics is required for FTTransformer metrics. Install torchmetrics to use this model."
		) from e
	...
	# Align preprocessing with ExcelFormer: CatToNum + MutualInformationSort.
	...
	if numerical_encoder_type == "linear":
		...
		raise ValueError(
			"Unsupported numerical_encoder. "
			"Expected one of: 'linear', 'linearbucket', 'linearperiodic'."
		)
	...
	nhead = int(config.get("nhead", config.get("num_heads", 8)))
...
def _tensor_frame_to_float_matrix(tensor_frame) -> torch.Tensor:
	...
	This follows the *token concatenation* order used by `StypeWiseFeatureEncoder`:
	iterate over `tensor_frame.stypes` and concatenate their mapped feature
	tensors along dim=1.
	"""
	...
	for st in tensor_frame.stypes:
		...
		if isinstance(feat, torch.Tensor):
			...
			# Fallback: try to coerce custom tensor types.
			try:
...
def feature_gnn_after_materialize_fn(
	...
	try:
		...
		raise ModuleNotFoundError(
			"Failed to import torch_frame.stype for feature topology init."
		) from e

	# After CatToNum + sorting, all usable features should be numerical.
...
def fttransformer_core_fn_row_gnn(material_outputs, config, task_type, gnn_stage=None):
	"""FTTransformer core function (row-GNN capable)."""

	print("Executing fttransformer_core_fn_row_gnn")
	...
	try:
		...
		raise ModuleNotFoundError("PyTorch is required to run FTTransformer.") from e
	...
	try:
		...
		raise ModuleNotFoundError(
			"Failed to import local torch_frame components required by fttransformer_core_fn_row_gnn."
		) from e
	...
	nhead = int(config.get("nhead", material_outputs.get("nhead", 8)))
	dropout = float(config.get("dropout", material_outputs.get("dropout", 0.2)))
	...
	if use_gnn:
		...
		if gnn_stage == "decoding":
			...
			print(
				f"[CORE] {label} stage uses Self-Attention + DGM_d + GCN, then decodes back to column tokens."
			)
			print(f"[CORE]  - attention_heads={attn_heads}, dgm_k={dgm_k}, dgm_distance={dgm_distance}")
	...
	def model_forward(tf):
		...
		if gnn_stage == "columnwise" and dgm_module is not None:
			...
			fusion_alpha = torch.sigmoid(fusion_alpha_param)
			x = x + fusion_alpha * tokens_out
			# Ensure injection affects readout.
			x_cls = x.mean(dim=1)
...
def fttransformer_core_fn_feature_gnn(material_outputs, config, task_type, gnn_stage=None):
	"""FTTransformer core function (feature graphify)."""

	print(f"Executing fttransformer_core_fn_feature_gnn with gnn_stage={gnn_stage}")

	stage = "none" if gnn_stage is None else str(gnn_stage).lower()
	if stage in {"none", "start", "materialize", "materialization"}:
		return fttransformer_core_fn_row_gnn(material_outputs, config, task_type, gnn_stage="none")
	...
	try:
		...
		raise ModuleNotFoundError(
			"Failed to import local torch_frame components required by fttransformer_core_fn_feature_gnn."
		) from e
	...
	def model_forward(tf):
		...
		if stage in {"columnwise", "decoding"}:
			if stage == "columnwise":
				x = _feature_inject(x)

		if stage == "columnwise":
			# Ensure injection affects readout by re-defining CLS.
			x_cls = x.mean(dim=1)
...
def main(train_df, val_df, test_df, dataset_results, config, gnn_stage):
	...
	print("FTTransformer - staged execution")
		...
		if graphify == "row":
			...
			if stage == "materialize":
				...
				try:
					...
					# Keep the original stype_encoder_dict if torch_frame import fails.
					pass

			results = fttransformer_core_fn_row_gnn(material_outputs, config, task_type, gnn_stage=stage)
			...
			results = fttransformer_core_fn_feature_gnn(
...
#  small+binclass
#  python main.py --models fttransformer --dataset kaggle_Audit_Data --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
#  small+regression
#  python main.py --models fttransformer --dataset openml_The_Office_Dataset --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
#  large+binclass
#  python main.py --models fttransformer --dataset credit --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
#  large+multiclass
#  python main.py --models fttransformer --dataset eye --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
#  python main.py --models fttransformer --dataset helena --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
#  large+regression
#  python main.py --models fttransformer --dataset house --gnn_stages all --graphify all --gpu 1 --epochs 2 --restore_best 0
